...
if(true){ 
  ...
  minDateForInput = dateNow.getFullYear()+'-'+(dateNow.getMonth()+1)+'-'+dateNow.getDate();
 ...
 $("select").selectBox().change(function() {
  ...
  if(isMobile.any()){
    $('.irs-hidden-input').attr("type", "button");
    if(isMobile.anyIOS())
    {
      $(".filter_datepicker").each(function(){
        $(this).attr("type", "date");
        $(this).attr("min", minDateForInput);
        $(this).val("");
        if($(this).parent().hasClass("input-line"))
        {
          $(this).parent().append('<label id="lbl' + $(this).attr("id") + '" for="' + $(this).attr("id") + '" style="position: absolute;color: #C2C2C3;height: 60px;padding: 13px;">' + $(this).attr("placeholder") + '</label>');
        }        
        else
        {
          var flag = false;
          if($(this).parent().hasClass("date-filters") && $(".filters-container").hasClass("hidden-sm")) {
            $(".search-results-container").animate({"margin-top": "20px"}, 300);
            $(".filters-container").css({"display": "none"});
            $(".filters-container").removeClass("hidden-sm hidden-xs");
            $(".filters-container").slideToggle('slow');
        ...
        $(this).focus(function(){
          if($("#lbl" + $(this).attr("id")).css("top").indexOf("px")==-1)
            $("#lbl" + $(this).attr("id")).css("top", $("#lbl" + $(this).attr("id")).position().top + "px");
      ...
      if($('input').is('#datepicker'))
      {
        ...
        $("#datepicker").attr("min", minDateForInput);
        $("#datepicker").parent().append('<label id="lbl' + $("#datepicker").attr("id") + '" for="datepicker" style="position: absolute;color: #C2C2C3;height: 60px;padding: 22px;left:' + $("#datepicker").offset().left + 'px;top:' + $("#datepicker").offset().top + 'px">' + $("#datepicker").attr("placeholder") + '</label>');
        $("#datepicker").val("");
      ...
      $("#start-date").attr("min", minDateForInput);
      $("#end-date").attr("type", "date");
      $("#end-date").attr("min", minDateForInput);
    }
    else
    {
...
function itemDetailsInit(){
  ...
  if($('div').is('.content-container')) {
    ...
    {
      var addTop = 0;
      if($('.aside__details').css('display')!='none')
        addTop = 300;
      if(($(this).scrollTop() + search_widget_height + addTop)>($('.content').height() + $('.content').offset().top)){       
        $('.search-aside .order-panel').css({ 'position': 'relative', 'top': $('.content').height() - search_widget_height - addTop - 20 });
      }
      else{
